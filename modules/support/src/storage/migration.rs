		...
		loop {
			...
			break match maybe_next {
				Some(next) => {
					...
					let maybe_value = frame_support2::storage::unhashed::get::<T>(&next);
					match maybe_value {
						Some(value) => {
							if self.drain {
								frame_support2::storage::unhashed::kill(&next);
							}
...
pub fn get_storage_value<T: Decode + Sized>(module: &[u8], item: &[u8], hash: &[u8]) -> Option<T> {
	...
	frame_support2::storage::unhashed::get::<T>(&key)
}
...
pub fn take_storage_value<T: Decode + Sized>(module: &[u8], item: &[u8], hash: &[u8]) -> Option<T> {
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	frame_support2::storage::unhashed::take::<T>(&key)
}
...
pub fn put_storage_value<T: Encode>(module: &[u8], item: &[u8], hash: &[u8], value: T) {
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	frame_support2::storage::unhashed::put(&key, &value);
}
...
pub fn remove_storage_prefix(module: &[u8], item: &[u8], hash: &[u8]) {
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	let _ = frame_support2::storage::unhashed::clear_prefix(&key, None, None);
}
...
pub fn clear_storage_prefix(
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	frame_support2::storage::unhashed::clear_prefix(&key, maybe_limit, maybe_cursor)
}
...
/// ```
/// # use frame_support2::storage::migration::move_storage_from_pallet;
...
/// "my_new_example_name", a migration can be:
/// ```
/// # use frame_support2::storage::migration::move_pallet;
/// # sp_io::TestExternalities::new_empty().execute_with(|| {
...
mod tests {
	...
	impl frame_support2::traits::StorageInstance for OldPalletStorageValuePrefix {
	...
	impl frame_support2::traits::StorageInstance for OldPalletStorageMapPrefix {
	...
	impl frame_support2::traits::StorageInstance for NewPalletStorageValuePrefix {
		const STORAGE_PREFIX: &'static str = "foo_value";
		fn pallet_prefix() -> &'static str {
			"my_new_pallet"
		}
	...
	impl frame_support2::traits::StorageInstance for NewPalletStorageMapPrefix {
		const STORAGE_PREFIX: &'static str = "foo_map";
		fn pallet_prefix() -> &'static str {
			"my_new_pallet"
		}
