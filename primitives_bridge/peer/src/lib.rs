...
mod our_chain_hash;
use crate::our_chain_hash::PeerHash;
...
/// Peer chain. This mostly depends on number of entries (and their density) in the storage trie.
/// Some reserve is reserved to account future chain growth.
pub const EXTRA_STORAGE_PROOF_SIZE: u32 = 1024;

/// Number of bytes, included in the signed Peer transaction apart from the encoded call itself.
///
...
/// Maximum weight of single Peer block.
///
...
/// Maximal number of unconfirmed messages in Peer confirmation transaction.
pub const MAX_UNCONFIRMED_MESSAGES_IN_CONFIRMATION_TX: MessageNonce = 128;

/// Weight of single regular message delivery transaction on Peer chain.
///
...
pub const DEFAULT_MESSAGE_DELIVERY_TX_WEIGHT: Weight = Weight::from_parts(1_500_000_000,14000);

/// Increase of delivery transaction weight on Peer chain with every additional message byte.
///
...
pub const ADDITIONAL_MESSAGE_BYTE_DELIVERY_WEIGHT: Weight = Weight::from_parts(25_000,14000);
/// Maximal weight of single message delivery confirmation transaction on Peer chain.
///
...
pub const MAX_SINGLE_MESSAGE_DELIVERY_CONFIRMATION_TX_WEIGHT: Weight = Weight::from_parts(2_000_000_000,14000);

/// Weight of pay-dispatch-fee operation for inbound messages at Peer chain.
///
...
pub const PAY_INBOUND_DISPATCH_FEE_WEIGHT: Weight = Weight::from_parts(700_000_000,14000);

/// The target length of a session (how often authorities change) on Peer measured in of number of
/// blocks.
///
...
/// Block number type used in Peer.
pub type BlockNumber = u32;

/// Hash type used in Peer.
pub type Hash = sp_core::H256;
/// Type of object that can produce hashes on Peer.
pub type Hasher = BlakeTwo256;

/// The header type used by Peer.
...
/// Weight-to-Fee type used by Peer.
...
impl sp_core::Hasher for BlakeTwoAndKeccak256 {
	...
	fn hash(s: &[u8]) -> Self::Out {
		let mut combined_hash = PeerHash::default();
...
/// We use this to get the account on Peer (target) which is derived from Rialto's (source)
/// account. We do this so we can fund the derived account on Peer at Genesis to it can pay
/// transaction fees.
///
...
pub fn derive_account_from_substrate_id(id: bp_runtime::SourceAccount<AccountId>) -> AccountId {
	let encoded_id = bp_runtime::derive_account_id(bp_runtime::SUBSTRATE_CHAIN_ID, id);
...
pub const WITH_PEER_GRANDPA_PALLET_NAME: &str = "BridgePeerGrandpa";
/// Name of the With-Millau messages pallet instance that is deployed at bridged chains.
pub const WITH_PEER_MESSAGES_PALLET_NAME: &str = "BridgePeerMessages";

/// Name of the Rialto->Millau (actually DOT->KSM) conversion rate stored in the Millau runtime.
pub const SUBSTRATE_TO_PEER_CONVERSION_RATE_PARAMETER_NAME: &str = "SubstrateToPeerConversionRate";

/// Name of the With-Rialto token swap pallet instance in the Millau runtime.
pub const WITH_SUBSTRATE_TOKEN_SWAP_PALLET_NAME: &str = "BridgeSubstrateTokenSwap";

/// Name of the `MillauFinalityApi::best_finalized` runtime method.
pub const BEST_FINALIZED_PEER_HEADER_METHOD: &str = "PeerFinalityApi_best_finalized";

/// Name of the `ToMillauOutboundLaneApi::estimate_message_delivery_and_dispatch_fee` runtime
/// method.
pub const TO_PEER_ESTIMATE_MESSAGE_FEE_METHOD: &str =
	"ToPeerOutboundLaneApi_estimate_message_delivery_and_dispatch_fee";
/// Name of the `ToMillauOutboundLaneApi::message_details` runtime method.
pub const TO_PEER_MESSAGE_DETAILS_METHOD: &str = "ToPeerOutboundLaneApi_message_details";

sp_api::decl_runtime_apis! {
	/// API for querying information about the finalized PEER headers.
	///
	/// This API is implemented by runtimes that are bridging with the PEER chain, not the
	/// PEER runtime itself.
	...
	///
	/// This API is implemented by runtimes that are sending messages to PEER chain, not the
	/// PEER runtime itself.
	pub trait ToPeerOutboundLaneApi<OutboundMessageFee: Parameter, OutboundPayload: Parameter> {
		/// Estimate message delivery and dispatch fee that needs to be paid by the sender on
		/// this chain.
		///
		/// Returns `None` if message is too expensive to be sent to PEER from this chain.
		///
