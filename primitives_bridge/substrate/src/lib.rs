...
/// Maximal weight of single Substrate block.
///
...
/// Maximal number of unrewarded relayer entries in Substrate confirmation transaction.
pub const MAX_UNREWARDED_RELAYERS_IN_CONFIRMATION_TX: MessageNonce = 1024;

/// Maximal number of unconfirmed messages in Substrate confirmation transaction.
pub const MAX_UNCONFIRMED_MESSAGES_IN_CONFIRMATION_TX: MessageNonce = 1024;

/// Weight of single regular message delivery transaction on Substrate chain.
///
...
/// Block number type used in Substrate.
pub type BlockNumber = u32;

/// Hash type used in Substrate.
pub type Hash = <BlakeTwo256 as HasherT>::Out;

/// The type of object that can produce hashes on Substrate.
pub type Hasher = BlakeTwo256;

/// The header type used by Substrate.
...
/// Weight-to-Fee type used by Substrate.
...
// We use this to get the account on Substrate (target) which is derived from Millau's (source)
// account. We do this so we can fund the derived account on Substrate at Genesis to it can pay
// transaction fees.
//
...
pub fn derive_account_from_peer_id(id: bp_runtime::SourceAccount<AccountId>) -> AccountId {
	let encoded_id = bp_runtime::derive_account_id(bp_runtime::PEER_CHAIN_ID, id);
...
/// Name of the With-Substrate GRANDPA pallet instance that is deployed at bridged chains.
pub const WITH_SUBSTRATE_GRANDPA_PALLET_NAME: &str = "BridgeSubstrateGrandpa";
/// Name of the With-Rialto messages pallet instance that is deployed at bridged chains.
pub const WITH_SUBSTRATE_MESSAGES_PALLET_NAME: &str = "BridgeSubstrateMessages";

/// Name of the Millau->Substrate (actually KSM->DOT) conversion rate stored in the Rialto runtime.
pub const PEER_TO_SUBSTRATE_CONVERSION_RATE_PARAMETER_NAME: &str = "PeerToSubstrateConversionRate";

/// Name of the parachain registrar pallet in the Substrate runtime.
pub const PARAS_REGISTRAR_PALLET_NAME: &str = "Registrar";

/// Name of the parachains pallet in the Substrate runtime.
pub const PARAS_PALLET_NAME: &str = "Paras";

/// Name of the `SubstrateFinalityApi::best_finalized` runtime method.
pub const BEST_FINALIZED_SUBSTRATE_HEADER_METHOD: &str = "SubstrateFinalityApi_best_finalized";

/// Name of the `ToRialtoOutboundLaneApi::estimate_message_delivery_and_dispatch_fee` runtime
/// method.
pub const TO_SUBSTRATE_ESTIMATE_MESSAGE_FEE_METHOD: &str =
	"ToSubstrateOutboundLaneApi_estimate_message_delivery_and_dispatch_fee";
/// Name of the `ToRialtoOutboundLaneApi::message_details` runtime method.
pub const TO_SUBSTRATE_MESSAGE_DETAILS_METHOD: &str = "ToSubstrateOutboundLaneApi_message_details";

sp_api::decl_runtime_apis! {
	/// API for querying information about the finalized Substrate headers.
	///
	/// This API is implemented by runtimes that are bridging with the Substrate chain, not the
	/// Millau runtime itself.
	pub trait SubstrateFinalityApi {
		/// Returns number and hash of the best finalized header known to the bridge module.
		fn best_finalized() -> Option<bp_runtime::HeaderId<Hash, BlockNumber>>;
	}

	/// Outbound message lane API for messages that are sent to Substrate chain.
	///
	/// This API is implemented by runtimes that are sending messages to Substrate chain, not the
	/// Rialto runtime itself.
	pub trait ToSubstrateOutboundLaneApi<OutboundMessageFee: Parameter, OutboundPayload: Parameter> {
		/// Estimate message delivery and dispatch fee that needs to be paid by the sender on
		/// this chain.
		///
		/// Returns `None` if message is too expensive to be sent to Substrate from this chain.
		///
